...
// 注意：不能收裸「不要」「不用」——「不要辣」「不要蒜」是點餐規格，
// 不是取消！包含式只收有結尾詞的否定；整句恰好是「不要／不用」才算。
export const NO_WORDS = [
  "取消", "娶消", "去消", "曲消", "取笑", "驅消", "不要了", "不用了",
  "算了", "不對", "錯了", "重來", "重新", "不下單", "不要下單", "別下單", "不買了",
];
const NO_EXACT = ["不要", "不用", "不", "否"];
export const EXIT_WORDS = [
  "結束對話", "結束", "結速", "傑束", "節束", "介束", "離開", "再見",
  "掰掰", "拜拜", "先這樣", "先醬", "不點了", "關閉", "關掉",
];

// 簡→繁字表（指令詞會用到的字）：ElevenLabs Scribe 等 STT 的中文輸出
// 可能是簡體（「确认下单」），繁體詞表字面比對會全滅——判讀前先正規化。
// 點餐內容不受影響：封閉詞彙引擎按「拼音」比對，簡繁同音。
const S2T: Record<string, string> = {
  "确": "確", "认": "認", "单": "單", "订": "訂", "结": "結", "对": "對",
  "话": "話", "开": "開", "关": "關", "闭": "閉", "销": "銷", "别": "別",
  "错": "錯", "来": "來", "说": "說", "请": "請", "离": "離", "这": "這",
  "样": "樣", "点": "點", "买": "買", "没": "沒", "听": "聽", "见": "見",
  "钱": "錢", "还": "還", "么": "麼",
};

export function normalize(raw: string): string {
  return [...raw.replace(/[\s。，！？!?.,、]/g, "")]
    .map((ch) => S2T[ch] ?? ch).join("");
}
...
export function confirmIntent(raw: string): "yes" | "no" | "other" {
  ...
  if (NO_EXACT.includes(text)) return "no"; // 整句就是「不要」＝拒絕
