...
- ATLAS_FEED_MEANING：TWD_USD（feed 報價＝1 台幣值多少美元，預設）
                      或 USD_TWD（feed 報價＝1 美元值多少台幣）
...
def units_per_twd_from_price(price: float, meaning: str, multiplier: float) -> int:
    """feed 價格 → 1 台幣的 USDC 最小單位（6 位小數；視 USDC≈USD）。

    TWD_USD：price＝1 TWD 值多少 USD → units = price × 1e6
    USD_TWD：price＝1 USD 值多少 TWD → units = 1e6 ÷ price
...
async def units_per_twd() -> int | None:
    ...
    try:
        ...
        units = units_per_twd_from_price(price, ATLAS_FEED_MEANING, ATLAS_RATE_MULTIPLIER)
    except Exception:
