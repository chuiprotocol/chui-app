...
1. 手機 A 開 Slush → 內建瀏覽器（或直接開
   `slush://browse/<快樂鹽酥雞的https網址>`）→ 進快樂鹽酥雞官網。
2. 按住 🎙️ 說：「**我要一份鹽酥雞加辣**」。
3. 畫面顯示並唸出「加辣鹽酥雞，總共 65 元，確認嗎？」→ 按「以 USDC 付款」。
4. Slush 跳出交易（2.08 USDC）→ 確認簽名。
5. 收據出現 **✅ 鏈上驗證通過** 與 explorer 連結 → 點開給觀眾看
   （suiscan 上是真實交易編號；金額是 USDC、訂單內容只有一串雜湊）。
6. 指著面板：`parse → menu(adapter) → quote → order(經 adapter 開單 FC0001)
   → checkout → chain.verify → chain.verified → paid 通知`。
   同時商家A的終端機印出「🍗 [自家系統] FC0001 已收款，下鍋！」
   ——**這家店的舊系統一行程式碼都沒改**。

### 場景二：商家B（公版店面開的店）

1. 手機 B 開 Slush 內建瀏覽器 → 好喝奶茶店官網。
2. 按住 🎙️ 說：「**一杯珍珠奶茶半糖去冰**」。
3. 覆誦「中杯半糖去冰珍珠奶茶，總共 55 元，確認嗎？」→ USDC 付款 → 簽名。
4. 收據＋explorer 連結；面板這次顯示訂單直達（原生協議，沒有 adapter 跳板）。
5. 講一句：「這家店是**改一個 config JSON** 開起來的
   （apps/storefront-template/config/goodtea.json）。」

### 場景三：語音入口（一段語音、哪家都能點）

1. 任一手機開語音入口 App。
2. 說「我要一份鹽酥雞加辣」→ App 顯示「📍 快樂鹽酥雞（協議自動路由）」。
3. 再說「一杯珍珠奶茶半糖去冰」→ 「📍 好喝奶茶店」。
4. 挑一單付款走完 → 同一支 App、同一種體驗、兩家異質商家。
5. 面板出現 `route.rank：跨商家信心度排名` 的封包——這就是協議在路由。

### 加碼（展示「絕不猜」）

對任何一個入口說「呃…我要那個好吃的」→ 系統**提出澄清問題**而不是下單。
在支付情境，猜錯比多問一輪糟糕得多——這是協議規定的行為。
